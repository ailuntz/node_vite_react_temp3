...
function MyNewPage() {
  ...
  useEffect(() => {
    ...
    if (searchInput && header) {
      searchInput.addEventListener('focus', () => header.classList.add('wide'));
      searchInput.addEventListener('blur', () => header.classList.remove('wide'));
    }

    // 弹出菜单和遮罩层逻辑
    const statusButtons = document.querySelectorAll('.status-button:not(.open)');
    ...
    return () => {
      ...
      statusButtons.forEach((button) => button.removeEventListener('click', () => {}));
      if (popUpClose) popUpClose.removeEventListener('click', () => {});
    };
